...
Rotator GyroProc::process(unsigned long ts, Rotator rot, int output[2]) {
    ...
    if (_isStand) {
        ...
        float angularV = (ypr->pitch - _oldYpr.pitch) * _eff;       // 90 is an empirical extracted factor to adjust for real units
        float estSpeed = -oldSpeed - angularV;                      // We use robot_speed(t-1) or (t-2) to compensate the delay
        float fDelta   = ts - _oldTS;
        ...
        // if (ctr == 0) {
            LOG("P:%7.2f => %7.2f, aV:%7.2f, oV:%7.2f, estSpeed:%7.2f, _estSpeed:%7.2f, tA:%7.2f, out:%7.2f, _output:%7.2f, spd:%6d, %6d rot.p:%7.2f\n",
