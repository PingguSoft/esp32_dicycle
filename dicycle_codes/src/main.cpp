...
static int handleSerialCommand(unsigned long ts, int key) {
    ...
    // special functions
    //
#ifdef PIN_FS_BROWSER_SW
    if (digitalRead(PIN_FS_BROWSER_SW) == LOW) {
        while (digitalRead(PIN_FS_BROWSER_SW) == LOW);  // wait release
        key = '&';
    }
#endif

#ifdef PIN_BOOT_SW
    if (digitalRead(PIN_BOOT_SW) == LOW) {
        while (digitalRead(PIN_BOOT_SW) == LOW);        // wait release
        key = '*';
    }
#endif
    ...
    if (iMode > 0) {
        switch (iMode) {
            case 1:
                _cmdHandler.getRobot()->getHW()->calibrate(key);
                return -100;
    ...
    switch (key) {
        ...
        case 'y':
            _cmdHandler.getGyroProc()->decM();
            break;

        case 'Y':
            _cmdHandler.getGyroProc()->incM();
            break;

        case 'b':
            _cmdHandler.getGyroProc()->decE();
            break;

        case 'B':
            _cmdHandler.getGyroProc()->incE();
            break;
    }
    return key;
}
